...
class Config:
    ...
    def get_next_api_key(self):
        """চক্রাকারে তালিকা থেকে পরবর্তী API কী প্রদান করে।"""
...
# ... (Admin Command Handler and Settings handlers remain the same) ...
...
async def get_settings_markup_and_text():
    exp_status = "অন" if config.EXPLANATION_ENABLED else "অফ"
    prefix_status = "অন" if config.PREFIX_ENABLED else "অফ"
    
    exp_button_text = f"ব্যাখ্যা লিঙ্ক {'অফ' if config.EXPLANATION_ENABLED else 'অন'} করুন"
    prefix_button_text = f"প্রিফিক্স {'অফ' if config.PREFIX_ENABLED else 'অন'} করুন"

    text = (
        f"⚙️ বর্তমান সেটিংস:\n\n"
        f"• চ্যানেল আইডি: `{config.TARGET_CHANNEL}`\n"
        f"• প্রিফিক্স: `{config.PREFIX}` (অবস্থা: **{prefix_status}**)\n"
        f"• ব্যাখ্যা লিঙ্ক: `{config.EXPLANATION_LINK}` (অবস্থা: **{exp_status}**)"
    )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton(exp_button_text, callback_data="toggle_explanation")],
        [InlineKeyboardButton(prefix_button_text, callback_data="toggle_prefix")],
...
@app.on_callback_query(filters.regex(r"^(toggle_explanation|toggle_prefix|set_channel|set_prefix|set_link|close_settings)$"))
async def handle_settings_callbacks(client: Client, callback: CallbackQuery):
    ...
    if action == "toggle_explanation":
        config.EXPLANATION_ENABLED = not config.EXPLANATION_ENABLED
        await callback.answer(f"ব্যাখ্যা লিঙ্ক এখন {'অন' if config.EXPLANATION_ENABLED else 'অফ'}")
    
    elif action == "toggle_prefix":
        config.PREFIX_ENABLED = not config.PREFIX_ENABLED
        await callback.answer(f"প্রিফিক্স এখন {'অন' if config.PREFIX_ENABLED else 'অফ'}")

    elif action in ["set_channel", "set_prefix", "set_link"]:
        prompts = {
        ...
        return await callback.answer("সেটিংস বন্ধ করা হয়েছে")
    
    settings_text, settings_markup = await get_settings_markup_and_text()
    await callback.message.edit_text(settings_text, reply_markup=settings_markup)
...
# ========= MCQ Command (Updated with Round-Robin Logic) =========
    ...
    try:
        ...
        for block in question_blocks:
            try:
                ...
                poll_explanation = config.EXPLANATION_LINK if config.EXPLANATION_ENABLED else None
                ...
                poll_data = { "question": question_text, "options": options_list, "correct_option_id": correct_option_id, "option_labels": found_labels, "explanation": poll_explanation }
                created_polls[poll_message.id] = poll_data

                markup = InlineKeyboardMarkup([
                    [ InlineKeyboardButton("🗑️ ডিলিট", callback_data=f"delete_{poll_message.id}"), InlineKeyboardButton("✏️ এডিট", callback_data=f"edit_{poll_message.id}") ]
                ])
        ...
        await status_msg.edit_text(f"❌ API Key (...{api_key[-4:]}) এর দৈনিক কোটা শেষ। পরবর্তী ছবিতে অন্য কী স্বয়ংক্রিয়ভাবে চেষ্টা করা হবে।")
    except Exception as e:
